...
void TestApp_Init(void)
{
    ...
    uint_32 i;
    DisableInterrupts;
    current_class=VIDEO_CLASS;
    for(i=0;i<1000;i++)
    {
        if(0==(MCF_GPIO_SETTA&1))
        {
    ...
    if(VIDEO_CLASS== current_class) {
        ...
        total_block= *((uint_32*)lba_data.buff_ptr) ;
        num_image= *((uint_32*)(lba_data.buff_ptr + 4)) +1 ;
        frame_rate= *((uint_32*)(lba_data.buff_ptr + 8)) ;
        ...
        /* Read the first block to get size_video*/
         //SD_Read_Block(&lba_data);
         //for(i=0;i<1000;i++)
            //for(j=0;j<1000;j++);
