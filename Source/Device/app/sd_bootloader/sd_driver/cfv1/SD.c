...
/************************************************/
/************************************************/
void init_sample_block(PTR_LBA_APP_STRUCT *lba_data_ptr, uint_32 offset,uint_32 size)
{
    /* Body */
    uint_32 i;
    uint_8 c=0;
    for(i=0;i<size;i++)
    {
        (*((*lba_data_ptr)->buff_ptr))=c;
        ((*lba_data_ptr)->buff_ptr)++;
        c++;
    } /* EndFor */
    (*lba_data_ptr)->offset = offset;
    (*lba_data_ptr)->size = size;
} /* EndBody */
/***********************************************/
